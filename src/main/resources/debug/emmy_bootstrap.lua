...
Три способа запуска, все выставляет сам плагин:

  * скрипт   — tarantool -e "dofile('…/emmy_bootstrap.lua')" script.lua
               (чанк -e выполняется до скрипта);
  * кластер  — TT_ROLES='["emmy_bootstrap", …роли инстанса]' tt start
               (файл подключается ролью, первой в списке: ядро загружает
               роли до validate и apply остальных, и точки останова
               работают уже в применении конфигурации; настоящие app.file
               и app.module ядро запускает само, как обычно);
  * кластер без выбранного инстанса — TT_APP_FILE=…/emmy_bootstrap.lua
               (переменная перекрывает app.file кластерной конфигурации;
               настоящее приложение загружается отсюда же — запасной путь,
               роли к этому моменту уже применены).
...
-- Ролью файл тоже годится: подключение уже случилось при загрузке модуля,
-- а ядру нужна таблица с тремя действиями. Повторное применение
-- конфигурации роль не трогает — отладчик подключён один раз на процесс.
return {
