...
* Lua-файлы (`emmy_debug.lua`, `emmy_bootstrap.lua`, `emmy_roles.lua`)
  распаковываются в системный каталог IDE, поэтому в проекте их держать
  не нужно;
* агент подставляется в процесс без правки кода: скрипту — чанком `-e`
  (он выполняется до самого скрипта), кластеру — ролью `emmy_bootstrap`
  первой в списке ролей инстанса через переменную `TT_ROLES` (каталог
  загрузчика добавляется в `LUA_PATH`); список настоящих ролей инстанса
  печатает `emmy_roles.lua` тем же модулем ядра, каким собирается
  конфигурация инстанса (`internal.config.cluster_config`), — разбирать
  `config.yaml` в IDE ненадёжно. Запасной путь без выбранного инстанса —
  подмена `app.file` на загрузчик через `TT_APP_FILE`, тогда настоящее
  приложение загрузчик выполняет сам;
