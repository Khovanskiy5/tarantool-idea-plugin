...
* **Графическая отладка одной кнопкой** (нужен EmmyLua2) — Debug
  у конфигурации «Tarantool» для скриптов и «Запустить с отладчиком»
  на панели инстансов для кластера. Плагин сам поднимает агент
  отладчика, ждёт порт и подключает сессию: код приложения править
  не нужно. Точки останова работают в любом файбере, в обработчиках
  iproto-запросов и внутри триггеров спейсов — включая стартовый код
  инстанса, потому что запуск приложения придерживается до подключения
  IDE. Таплы и прочие cdata смотрятся в Watches через `D(tuple)`.
...
* **Шаблоны страниц tnt-template** — файлы `*.thtml.lua` (аналог
  `.blade.php`) открываются как шаблон над HTML, а не как Lua:
  подсветка тегов от HTML-плагина, вкраплений `{{ }}`, `{!! !!}`,
  `{{-- --}}` и `@директив` — своя, комментирование блоком `{{-- --}}`,
  дополнение директив после `@`, Lua внутри выражений — инъекцией
  (при установленном EmmyLua2).
