...
* **Встроенный отладчик** — флажок «tarantool -d»: скрипт стартует под
  интерактивной консолью luadebug прямо в окне Run. Графическая
  отладка — через плагин EmmyLua2 (Emmy Debugger).
...
* **Схемы конфигурации** — автодополнение и валидация `config.yaml`
  (официальная JSON Schema кластерной конфигурации Tarantool 3)
  и `tt.yaml`.
* **Типы для EmmyLua2** — генерация описаний API интроспекцией
  установленного Tarantool плюс уточнённые вручную сигнатуры:
  автодополнение по `box`, `fiber`, `net.box`.
